...
- **Middleware/GlobalExceptionMiddleware.cs** → Catches ArgumentException (400), KeyNotFoundException (404), InvalidOperationException (409).
...
- **src/stores/** → Pinia stores (auth, beneficiarios)
- **src/views/** → Page components. **src/components/** → Reusable components.
